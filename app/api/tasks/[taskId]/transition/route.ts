...
export async function POST(
  ...
  try {
    ...
    if (condition.owner_only && task.owner !== userId) {
      return NextResponse.json(
        { error: 'Task 소유자만 이 전환을 수행할 수 있습니다.' },
        { status: 403 }
      )
    }
    ...
    await pool.query(
      'UPDATE project_children SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [newStatus, taskId]
    )

    return NextResponse.json({
      success: true,
      message: `상태가 ${currentStatus}에서 ${newStatus}로 변경되었습니다.`,
      task_id: taskId,
