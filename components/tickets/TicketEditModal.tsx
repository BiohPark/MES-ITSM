...
const DETAIL_FIELDS: Record<TicketType, Array<{ key: string; label: string; placeholder: string }>> = {
  request: [
    { key: 'requested_service', label: '요청 서비스', placeholder: '예: 계정 생성' },
    { key: 'business_reason', label: '요청 사유', placeholder: '업무 목적을 입력하세요' },
  ],
  incident: [
    { key: 'affected_service', label: '영향 서비스', placeholder: '장애가 발생한 서비스' },
    { key: 'symptom', label: '증상', placeholder: '사용자 영향/증상' },
  ],
  problem: [
    { key: 'root_cause_hypothesis', label: '원인 가설', placeholder: '근본 원인 가설' },
    { key: 'recurrence_pattern', label: '재발 패턴', placeholder: '재발 주기/패턴' },
  ],
  change: [
    { key: 'change_scope', label: '변경 범위', placeholder: '예: DB 스키마 변경' },
    { key: 'rollback_plan', label: '롤백 계획', placeholder: '실패 시 복구 절차' },
  ],
}
...
export function TicketEditModal({ ticket, mode, currentUser, onClose, onSave }: TicketEditModalProps) {
  ...
  const detailFieldDefs = DETAIL_FIELDS[formData.ticket_type]
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '1440px', width: 'min(1440px, calc(100vw - 2rem))', maxHeight: '92vh', overflowY: 'auto' }}>
        <div className="modal-header">
          <h2>{mode === 'edit' ? `${TYPE_LABEL[formData.ticket_type]} 상세` : `${TYPE_LABEL[formData.ticket_type]} 등록`}</h2>
        ...
        <form id="ticket-form" onSubmit={handleSubmit} className="project-form" style={formContainerStyle}>
          <div className="form-row">
            <div className="form-group">
              <label htmlFor="ticket_no">번호</label>
              <input id="ticket_no" name="ticket_no" value={formData.ticket_no} className="form-input" disabled />
            </div>
            <div className="form-group">
              <label htmlFor="ticket_type">유형</label>
              <input id="ticket_type" name="ticket_type" value={TYPE_LABEL[formData.ticket_type]} className="form-input" disabled />
            </div>
            {supportsApproval && (
              <div className="form-group">
                <label htmlFor="approver_name">승인자</label>
                <input id="approver_name" name="approver_name" value={formData.approver_name || ''} onChange={handleChange} className="form-input" placeholder="결재자 이름" />
              </div>
            )}
          </div>
          ...
          <div className="form-row">
            <div className="form-group">
              <label htmlFor="related_project_id">관련 프로젝트</label>
            ...
            <div className="form-group">
              <label htmlFor="catalog_item_id">카탈로그 ID</label>
          ...
          <div className="form-row">
            <div className="form-group">
              <label>SLA</label>
              <input value={`${formData.sla_status || '-'} / ${formData.sla_due_at ? new Date(formData.sla_due_at).toLocaleString('ko-KR') : '-'}`} className="form-input" disabled />
            </div>
            <div className="form-group">
              <label>승인 상태</label>
              <input value={supportsApproval ? formData.approval_status || 'N/A' : '미사용'} className="form-input" disabled />
            </div>
            <div className="form-group">
              <label htmlFor="opened_at">접수 시각</label>
        ...
        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>운영 정책 상태</h3>
          ...
          {supportsApproval && formData.approval_required && formData.approval_status !== 'Requested' && formData.approval_status !== 'Approved' && (
            <div style={{ marginTop: '0.75rem' }}>
              <button type="button" className="btn btn-primary" onClick={() => void requestApproval()}>승인 요청</button>
            </div>
          )}
        </section>

        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>관련 항목 링크</h3>
          {formData.id && (
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
              <input className="form-input" style={{ flex: '0 0 140px' }} value={linkForm.link_type} onChange={(e) => setLinkForm((prev) => ({ ...prev, link_type: e.target.value }))} placeholder="link type" />
              <input className="form-input" style={{ flex: '0 0 140px' }} value={linkForm.linked_entity_type} onChange={(e) => setLinkForm((prev) => ({ ...prev, linked_entity_type: e.target.value }))} placeholder="entity type" />
              <input className="form-input" style={{ flex: '1 1 220px', minWidth: 0 }} value={linkForm.linked_entity_id} onChange={(e) => setLinkForm((prev) => ({ ...prev, linked_entity_id: e.target.value }))} placeholder="entity id" />
              <input className="form-input" style={{ flex: '1 1 260px', minWidth: 0 }} value={linkForm.linked_entity_label || ''} onChange={(e) => setLinkForm((prev) => ({ ...prev, linked_entity_label: e.target.value }))} placeholder="표시 라벨" />
              <button type="button" className="btn btn-secondary" onClick={() => void addLink()}>추가</button>
            </div>
          )}
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            {(formData.links || []).length === 0 ? (
              <div style={{ color: '#64748b' }}>등록된 링크가 없습니다.</div>
            ) : (
              (formData.links || []).map((link) => (
                <div key={link.id} style={{ padding: '0.75rem', border: '1px solid #e5e7eb', borderRadius: 8, display: 'flex', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
                  ...
                  <button type="button" className="btn btn-secondary" onClick={() => void deleteLink(link.id)}>삭제</button>
                </div>
              ))
            )}
          </div>
        ...
        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>활동 이력</h3>
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            {(formData.activity_logs || []).length === 0 ? (
              <div style={{ color: '#64748b' }}>활동 이력이 없습니다.</div>
            ) : (
              (formData.activity_logs || []).map((log) => (
                <div key={log.id} style={{ padding: '0.75rem', border: '1px solid #e5e7eb', borderRadius: 8, overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
                  ...
                  <div style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '0.35rem' }}>
                    {log.actor_name || '-'} / {log.created_at ? new Date(log.created_at).toLocaleString('ko-KR') : '-'}
                  </div>
                </div>
              ))
            )}
        ...
        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>승인 이력</h3>
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            {(formData.approvals || []).length === 0 ? (
              <div style={{ color: '#64748b' }}>승인 이력이 없습니다.</div>
            ) : (
        ...
        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>에스컬레이션</h3>
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            {(formData.escalations || []).length === 0 ? (
              <div style={{ color: '#64748b' }}>에스컬레이션 이력이 없습니다.</div>
            ) : (
        ...
        <section style={sectionContainerStyle}>
          <h3 style={{ marginBottom: '0.75rem' }}>감사 로그</h3>
          <div style={{ display: 'grid', gap: '0.5rem' }}>
            {(formData.audit_logs || []).length === 0 ? (
              <div style={{ color: '#64748b' }}>감사 로그가 없습니다.</div>
            ) : (
              (formData.audit_logs || []).map((item) => (
                <div key={item.id} style={{ padding: '0.75rem', border: '1px solid #e5e7eb', borderRadius: 8, overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
                  <strong>{item.action}</strong> / {item.message}
                  <div style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '0.35rem' }}>
                    {item.actor_name || '-'} / {item.created_at ? new Date(item.created_at).toLocaleString('ko-KR') : '-'}
                  </div>
                </div>
              ))
            )}
        ...
        {formData.id && currentUser && (
          <div style={sectionContainerStyle}>
            <AttachmentSection
              ...
              titleHint="증빙 자료, 변경계획서, 로그 등을 첨부하세요."
            />
          </div>
        )}
