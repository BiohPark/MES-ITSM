...
MySQL 데이터베이스 파일은 MySQL 설치 방법에 따라 다른 위치에 저장됩니다:

### 1. MySQL 공식 설치 프로그램 사용 시
```
C:\ProgramData\MySQL\MySQL Server X.X\Data\project_management\
```

여기서 `X.X`는 MySQL 버전 번호입니다 (예: 8.0, 8.4 등)
...
### 방법 2: MySQL 설정 파일 확인
MySQL 설정 파일(`my.ini` 또는 `my.cnf`)에서 `datadir` 항목을 확인하세요.
...
데이터베이스 파일은 MySQL 서버의 데이터 디렉토리 내에 저장되며, 프로젝트 폴더 내에는 저장되지 않습니다.
